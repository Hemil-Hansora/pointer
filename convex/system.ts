...
//use for Agent conversation context
...
//use for Agent to update conversation title
...
export const createFile = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (existing) {
      throw new Error(
        `File with name "${args.name}" already exists in this folder.`,
      );
    }
...
export const createFiles = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    for (const file of args.files) {
      const exiting = existingFiles.find(
