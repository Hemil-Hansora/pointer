...
export const exportToGithub = inngest.createFunction(
  ...
  async ({ event, step }) => {
    const { githubToken, projectId, repoName, visibility, description } =
      event.data as ExportToGithubEvent;

    const internalKey = process.env.POINTER_CONVEX_INTERNAL_KEY;
    if (!internalKey) {
      throw new NonRetriableError(
        "POINTER_CONVEX_INTERNAL_KEY is not configured",
      );
    }
    ...
    const { data: repo } = await step.run("create-repo", async () => {
      return await octokit.rest.repos.createForAuthenticatedUser({
        name: repoName,
        description: description || "Exported from Pointer",
    ...
    const { data: commit } = await step.run("create-commit", async () => {
      return await octokit.rest.git.createCommit({
        owner: user.login,
        repo: repoName,
        message: "Initial commit from Pointer",
        tree: tree.sha,
    ...
    await step.run("set-complete-status", async () => {
      await convex.mutation(api.system.updateExportStatus, {
        internalKey,
        projectId,
        status: "completed",
