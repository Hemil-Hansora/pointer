...
const ProjectItem = ({ data }: { data: Doc<"projects"> }) => {
  return (
    <Link
      ...
      className="text-sm text-foreground/60  font-medium py-1 flex items-center justify-between w-full group hover:text-foreground/90"
    >
      ...
      <span className="text-xs text-muted-foreground group-hover:text-foreground/60 transition-colors">
        {formateTimestamp(data.updatedAt)}
      </span>
      {/* <ArrowRightIcon /> */}
    </Link>
  );
