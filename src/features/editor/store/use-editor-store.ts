...
export const useEditorStore = create<EditorStore>()((set, get) => ({
  ...
  openFile: (projectId, fileId, { pinned }) => {
    ...
    // Case 1 : Opening a file as a preview  -- replace existing preview or add as new tab
    if (!isOpen && !pinned) {
    ...
    /// Case 2: Opening a file as pinned -- add new tab
    if (!isOpen && pinned) {
      tabs.set(projectId, {
        ...state,
    ...
    // Case 3: File is already open -- just activate (and pin if needed)
  ...
  closeTab: (projectId, fileId) => {
    ...
    if (tabIndex === -1) return; // Tab not open
